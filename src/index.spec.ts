...
describe('Tests the public API', () => {
    ...
    it('detects 3g2', () => {
        ...
        const bytes = Array.prototype.slice.call(buffer, 0);
        expect(filetypeinfo(bytes)).toHaveLength(1);
        expect(filetypeinfo(bytes)[0].typename).toBe('3g2');
        expect(filetypeinfo(bytes)[0].mime).toBe('video/3gpp2');
    });
    it('detects 3gp', () => {
        const buffer = fs.readFileSync(require.resolve('./testfiles/a.3gp'));
        const bytes = Array.prototype.slice.call(buffer, 0);
        expect(filetypeinfo(bytes)).toHaveLength(1);
        expect(filetypeinfo(bytes)[0].typename).toBe('3gp');
        expect(filetypeinfo(bytes)[0].mime).toBe('video/3gpp');
    });
